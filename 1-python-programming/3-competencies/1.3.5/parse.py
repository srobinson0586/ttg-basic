...
See https://www.json.org for the complete explanation of JSON format.
...
- key-value pair in an object, and
- value in an array.
...
def parse_json(filename):
    ...
    You may assume the contents are contained in a single object whose
...
def parse_object(f):
    ...
    object. The keys will all be JSON strings. Each value can be
    - an integer,
    - a string,
    - an object, or
    - an array.
    ...
    2. call `parse_string` on the left side to get the key, and
    3. call `parse_value` on the right side to get the value (this might
       end up processing many additional lines from the file if the value
